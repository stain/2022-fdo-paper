...
\usepackage[backend=biber,alldates=iso,labeldate=year,style=authoryear,maxnames=2,maxbibnames=999,maxcitenames=2]{biblatex}
...
\item
  Tombstones for deleted resources are not well supported, nor specified, for either approach, although the continued availability of metadata when data is removed is a requirement for FAIR principles (see RDA-A2-01M in Table \vref{RDA-A2-01M} introduced by section \emph{Assessing FDO against FAIR}).
...
An earlier version of this article was published as a preprint \autocite{soilandreyes2023evaluating}, which is embedded in the corresponding RO-Crate for this article \autocite{soilandreyes2023} and the PhD thesis \autocite{soilandreyesthesis}.
