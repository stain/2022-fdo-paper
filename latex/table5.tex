...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2}}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2}}@{}}
\caption{Assessing RDA's FAIR Data Maturity Model \autocite{groupFAIRDataMaturity2020,bahimFAIRDataMaturity2020a} (first 2 columns) against the FDO guidelines \autocite{boninoFAIRDigitalObject}, FDO implemented with the protocol DOIPv2 \autocite{foundationDigitalObjectInterface}, Linked Data Platform (LDP) \autocite{FDOFramework} and examples from Linked Data practices in general. (--- indicates \emph{Unspecified}, may be possible with additional conventions)
